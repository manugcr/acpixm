...
def analyze(
    ...
    *,
    ...
    for i, target in enumerate(targets, 1):
        ...
        if logic_rule:
            logger.info(
                "Applying logic rules to %d records from %s",
                len(normalized_matches),
                target.name,
            )
            records = LogicEngine(logic_rule, external_vars).evaluate(
                normalized_matches
            )
            logger.debug(
                "Logic evaluation resulted in %d records for %s",
                len(records),
                target.name,
            )
        else:
        ...
        logger.debug(
            "Return evaluation: %d/%d records kept from %s",
            len(kept_decisions),
            len(decisions),
            target.name,
        )
        ...
        for decision in decisions:
            formatter.feed(MatchEvent(rule=rule_info, target=target, decision=decision))

    # Stage 5: Generate final output summary
    logger.info(
        "Analysis completed: %d total matches across %d files",
        total_matches,
        len(targets),
    )
