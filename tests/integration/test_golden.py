...
if shutil.which("acpixm") is None:
    pytest.skip(
        "acpixm CLI not installed (run via `uv run pytest`)", allow_module_level=True
    )
...
def test_opregion_low_mem_flags_only_low_address():
    ...
    findings = _analyze_json(
        FIXTURES / "OpRegionLowMem.yml", FIXTURES / "low_mmio.asl"
    )["findings"]
    assert len(findings) == 1, f"expected exactly 1 finding, got {len(findings)}"
...
def test_opregion_low_mem_does_not_flag_rootkit():
    ...
    findings = _analyze_json(
        FIXTURES / "OpRegionLowMem.yml", FIXTURES / "rootkit1.asl"
    )["findings"]
    assert len(findings) == 0, (
        f"expected 0 findings for rootkit1.asl, got {len(findings)}"
    )
